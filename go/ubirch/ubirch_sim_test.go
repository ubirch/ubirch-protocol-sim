...
func TestSim_GenerateSourceRandom(t *testing.T) {
	asserter := assert.New(t)
	requirer := require.New(t)
	...
	// test Generate Source Random APDU // [1] 2.1.3
	randBytes, err := sim.Random(32)
	asserter.NoErrorf(err, "failed to generate random number")
	log.Printf("RANDOM: %s", hex.EncodeToString(randBytes))
}
